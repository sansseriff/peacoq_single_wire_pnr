...
class CoincidenceExample(QMainWindow):
    ...
    def updateMeasurements(self):
        ...
        if self.ent:
            ...
            try:
                # max = numpy.max(hist1)
                max = self.ui.correlationBins.value()
                # print("max: ", numpy.max(hist1))
            except:
                max = 0
    ...
    def handleScanInput(self):
        ...
        res = input("input vertical resolution (number or 'max'): ")
        if res == "max":
            self.trigger_levels = numpy.arange(start, end, 0.0001).tolist()
        else:
            self.trigger_levels = numpy.arange(start, end, float(res)).tolist()

        self.save_time = float(input("save time per level: "))
        self.file_append = input("addend to file name: ")
    ...
    def triggerJitterScan(self):
        ...
        max = self.ui.correlationBins.value()
        self.bins = numpy.arange(1, max)
        save_data = {"bins": self.bins.tolist(), "data_list": []}
    ...
    def load_file_params(self):
        ...
        self.ent = False
        with open("./peacoq_UI_params.yaml", "r", encoding="utf8") as stream:
            try:
    ...
    def export_file_params(self):
        ...
        ui_data["Channels"]["ChA"]["channel"] = int(self.ui.channelA.value())
        ui_data["Channels"]["ChA"]["trigger"] = self.ui.triggerA.value()
        ui_data["Channels"]["ChA"]["delay"] = int(self.ui.delayA.value())
        ui_data["Channels"]["ChA"]["dead_time"] = self.ui.deadTimeA.value()

        ui_data["Channels"]["ChB"]["channel"] = int(self.ui.channelB.value())
        ui_data["Channels"]["ChB"]["trigger"] = self.ui.triggerB.value()
        ui_data["Channels"]["ChB"]["delay"] = int(self.ui.delayB.value())
        ui_data["Channels"]["ChB"]["dead_time"] = self.ui.deadTimeB.value()

        ui_data["Channels"]["ChC"]["channel"] = int(self.ui.channelC.value())
        ui_data["Channels"]["ChC"]["trigger"] = self.ui.triggerC.value()
        ui_data["Channels"]["ChC"]["delay"] = int(self.ui.delayC.value())
        ui_data["Channels"]["ChC"]["dead_time"] = self.ui.deadTimeC.value()

        ui_data["Channels"]["ChD"]["channel"] = int(self.ui.channelD.value())
        ui_data["Channels"]["ChD"]["trigger"] = self.ui.triggerD.value()
        ui_data["Channels"]["ChD"]["delay"] = int(self.ui.delayD.value())
        ui_data["Channels"]["ChD"]["dead_time"] = self.ui.deadTimeD.value()

        ui_data["Histogram"]["bins"] = int(self.ui.correlationBins.value())
        ui_data["Histogram"]["bin_width"] = int(self.ui.correlationBinwidth.value())
